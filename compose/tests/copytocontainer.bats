...
setup() {
  "$(magento_docker_base_path)"/bin/cli rm -f "${DUMMY_TEST_FILE_PATH}"
  "$(magento_docker_base_path)"/bin/cli rm -rf "${DUMMY_TEST_DIR}"
}
...
@test 'copytocontainer: copy a single file to a nested directory' {
  ...
  run "$(magento_docker_base_path)"/bin/cli ls -l "$(dirname "${DUMMY_TEST_FILE_PATH}")"
  [ "${status}" -eq 0 ]
  [[ "${lines[@]}" =~ 'test-file.txt' ]]
}
